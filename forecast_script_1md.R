...
df <- read_csv2("forecast_data_2020-03-12.csv") %>%
  ...
  mutate(Tilgang = if_else(month==yearmonth("2019 Jul"), Tilgang - 6000,Tilgang))
...
streaming_ci <- fc_streaming %>% 
  select(contains("ci")) %>%
  unlist() %>% 
  tibble::enframe() %>%
  filter(!str_detect(name,"level")) %>%
  pivot_wider() %>%
  mutate(month = fc_streaming$month)
  
fc_streaming <- fc_streaming %>%
  select(-contains("ci")) %>%
  inner_join(streaming_ci)
...
# clean confidence intervals
fc_betalende_ci <- fc_betalende_start %>% 
  select(contains("ci")) %>%
  unlist() %>% 
  tibble::enframe() %>%
  filter(!str_detect(name,"level")) %>%
  pivot_wider() %>%
  mutate(month = fc_betalende_start$month)

fc_betalende_start <- fc_betalende_start %>%
  select(-contains("ci")) %>%
  inner_join(fc_betalende_ci)
...
# clean confidence intervals
fc_betalende_ci <- fc_betalende_slut  %>% 
  select(contains("ci")) %>%
  unlist() %>% 
  tibble::enframe() %>%
  filter(!str_detect(name,"level")) %>%
  pivot_wider() %>%
  mutate(month = fc_betalende_slut$month)

fc_betalende_slut  <- fc_betalende_slut  %>%
  select(-contains("ci")) %>%
  inner_join(fc_betalende_ci)
...
# clean confidence intervals
fc_samlet_start_ci <- fc_samlet_start  %>% 
  select(contains("ci")) %>%
  unlist() %>% 
  tibble::enframe() %>%
  filter(!str_detect(name,"level")) %>%
  pivot_wider() %>%
  mutate(month = fc_samlet_start$month)

fc_samlet_start  <- fc_samlet_start  %>%
  select(-contains("ci")) %>%
  inner_join(fc_samlet_start_ci)
...
# clean confidence intervals
fc_samlet_slut_ci <- fc_samlet_slut  %>% 
  select(contains("ci")) %>%
  unlist() %>% 
  tibble::enframe() %>%
  filter(!str_detect(name,"level")) %>%
  pivot_wider() %>%
  mutate(month = fc_samlet_slut$month)

fc_samlet_slut  <- fc_samlet_slut  %>%
  select(-contains("ci")) %>%
  inner_join(fc_samlet_slut_ci)
...
# sammenlign start
tibble(Gratis_start_beregn.,fc_gratis =fc_gratis_start$Gratis_start,month = 1:length(Gratis_start_beregn.)) %>%
  pivot_longer(-month) %>%
  ggplot(aes(month,value,colour=name)) +
  geom_line()

# sammenlign slut
tibble(Gratis_slut_beregn.,fc_gratis =fc_gratis_slut$Gratis_slut,month = 1:length(Gratis_slut_beregn.)) %>%
  pivot_longer(-month) %>%
  ggplot(aes(month,value,colour=name)) +
  geom_line()
...
output <- forecasts %>% 
  ...
  select(month:Betalende_slut_ci80..upper,
         Gratis_start_beregn.:Gratis_slut_beregn.,
         SamletBestand_start:AntalStreamere,
         AndelStreamere,
         Revenue:ChurnRate2) %>% 
  mutate_at(vars(contains("M_Buckets")),function(.) {. *100}) %>%
  mutate_at(vars(contains("ci"),Betalende_start,Betalende_slut,
...
plots_filename <- glue::glue("forecast_grafer_{month.name[month(today())]}.png")

ggsave(plots_filename,plots,width=14,height=9)

data <- list(Forecasts = output, Evaluering = accuracy_output)

filename <- glue::glue("C:/Users/pech/Desktop/RProjects/ForecastAbo/forecasts/forecasts_{month.name[month(today())]}.xlsx")

#write.xlsx(output, file = filename)
write.xlsx(data, file = glue::glue("testfil_md_forecasts_{month.name[month(today())]}.xlsx"))
...
email <- envelope() %>%
  ...
  to("pech@saxo.com") %>% 
  cc("pech@saxo.com") %>%
  subject(glue::glue("Forecasts {month.name[month(today())]}")) %>%
